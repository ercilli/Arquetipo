...
namespace Tracing.Configuration.Services
{
	public class JaegerPropagator : TextMapPropagator
	{
        ...
        private readonly OpenTelemetry.Extensions.Propagators.B3Propagator b3Propagator = new OpenTelemetry.Extensions.Propagators.B3Propagator();

        public override ISet<string> Fields => new HashSet<string>(b3Propagator.Fields);
        ...
        private ActivityContext ParseUberTraceId(string uberTraceIdEncoded)
        {
            // Decodifica el uber-trace-id que está codificado para URL
            var uberTraceId = Uri.UnescapeDataString(uberTraceIdEncoded);

            var parts = uberTraceId.Split(':');

            if (parts.Length < 4)
            {
                throw new ArgumentException("Formato de uber-trace-id inválido.");
            }
            ...
            var traceFlags = flags == "1" ? ActivityTraceFlags.Recorded : ActivityTraceFlags.None;
